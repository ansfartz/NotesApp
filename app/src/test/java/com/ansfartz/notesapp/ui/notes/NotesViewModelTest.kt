...
class NotesViewModelTest {
    ...
    @Test
    fun `initial state contains five sample notes`() {
    ...
    fun `sample notes have IDs 1 through 5`() {
        val ids = viewModel.notes.value.map { it.id }.sorted()
    ...
    fun `saveNote update preserves list order`() {
        val originalIds = viewModel.notes.value.map { it.id }
        val updated = Note(id = 3L, title = "Updated Ideas", body = "New body", color = NoteColors.all[0])
        viewModel.saveNote(updated)

        assertEquals(originalIds, viewModel.notes.value.map { it.id })
    }
    ...
    fun `deleteNote with nonexistent ID does not change list`() {
        val sizeBefore = viewModel.notes.value.size
        viewModel.deleteNote(999L)
        assertEquals(sizeBefore, viewModel.notes.value.size)
    }
    ...
    fun `save then delete then re-save produces correct state`() {
        ...
        assertEquals(6, viewModel.notes.value.size)

        viewModel.deleteNote(300L)
        assertEquals(5, viewModel.notes.value.size)
